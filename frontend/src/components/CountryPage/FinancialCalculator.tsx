...
import type { FinancialRequirements } from '../../types/country'

interface Props {
  requirements: FinancialRequirements
}

export default function FinancialCalculator({ requirements }: Props) {
  const [months, setMonths] = useState(12)
  const [extraAdults, setExtraAdults] = useState(0)
  const [children, setChildren] = useState(0)

  const ref = requirements.reference_value_monthly_eur ?? 0
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3 p-4 bg-blue-50 border border-blue-100 rounded-lg">
        <div>
          <p className="text-xs text-blue-600 uppercase tracking-wide font-medium">
            Reference Value {requirements.year ? `(${requirements.year})` : ''}
          </p>
      ...
      <div className="grid grid-cols-3 gap-3">
        <NumberInput label="Stay (months)" value={months}     min={1} max={60} onChange={setMonths} />
        <NumberInput label="Extra adults"  value={extraAdults} min={0} max={5}  onChange={setExtraAdults} />
        <NumberInput label="Children (<18)" value={children}  min={0} max={5}  onChange={setChildren} />
      </div>
      ...
      {requirements.acceptable_proof && requirements.acceptable_proof.length > 0 && (
        <div>
      ...
      {requirements.exemptions && requirements.exemptions.length > 0 && (
  ...
  return (
    <div>
      ...
      <div className="flex items-center border rounded-lg overflow-hidden">
        <button className="px-3 py-2 text-gray-500 hover:bg-gray-100 font-bold" onClick={() => onChange(Math.max(min, value - 1))}>−</button>
        <span className="flex-1 text-center font-semibold text-sm">{value}</span>
        <button className="px-3 py-2 text-gray-500 hover:bg-gray-100 font-bold" onClick={() => onChange(Math.min(max, value + 1))}>+</button>
      </div>
    </div>
  )
}
